...
/**
 * The GUI which shows the list of Containers the user made, and is able to go
 * to deleteContainerView, editContainerView, HomeView and ItemsView
 */
public class SeeContainersView implements ActionListener {
	...
	/**
	 * Sets the visibility of the SeeContainersView GUI depending on the boolean
	 * passed
	 * 
	...
	public void renameContainer(JButton b) {
		...
		if (c != null) {
			...
			ContainerUtility.verifyEditContainer(c.getName(), nameOfContainer, HomeView.data, b,
					HomeView.getContainerMap(), (errorMsg) -> JOptionPane.showMessageDialog(HomeView.getFrame(),
							errorMsg, "Input Error", JOptionPane.ERROR_MESSAGE),
					() -> {
	...
	public void removeContainer(JButton b) {
		...
		if (c != null) {
			...
			if (opt == JOptionPane.YES_OPTION) { // if not cancelled
				ContainerUtility.verifyDeleteContainer(c.getName(), HomeView.data, b, HomeView.getContainerMap(),
						(errorMsg) -> JOptionPane.showMessageDialog(HomeView.getFrame(), errorMsg, "Input Error",
