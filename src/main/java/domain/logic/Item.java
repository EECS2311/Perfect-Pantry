...
/**
 * Represents an item with a name, quantity, a set of food group tags, a food
 * freshness tag, and an expiration date. This class also implements the
 * Comparable interface to allow items to be sorted based on their expiration
 * date and name.
 */
public class Item implements Comparable<Item> {
	...
	// Constructors are private to enforce the use of factory methods for instance
	// creation.
	 ...
	 * @param foodGroupTag     A set of generic tags representing the food groups of
	 *                         the item.
	 ...
	 */
	public static Item getInstance(String name, GenericTag<FoodGroup> foodGroupTag,
			GenericTag<FoodFreshness> foodFreshnessTag, int quantity, Date expiryDate) {
	...
	/**
	 * Factory method to create an instance of Item with specified properties
	 * including a string representation of the expiry date.
	 *
	 * @param name             The name of the item.
	 * @param foodGroupTag     A set of generic tags representing the food groups of
	 *                         the item.
	 * @param foodFreshnessTag A generic tag representing the freshness of the item.
	 * @param quantity         The quantity of the item.
	 * @param expiryDate       A string representation of the expiration date in the
	 *                         format "dd-MMMM-yyyy".
	 * @return An instance of Item.
	 */
	public static Item getInstance(String name, GenericTag<FoodGroup> foodGroupTag,
			GenericTag<FoodFreshness> foodFreshnessTag, int quantity, String expiryDate) {
		Date date = parseDate(expiryDate);
		return Item.getInstance(name, foodGroupTag, foodFreshnessTag, quantity, date);
	}

	/**
	 * Factory method to create an Item instance with food groups and freshness
	 * defined by enums.
	 *
	 ...
	 */
	public static Item getInstance(String name, FoodGroup foodGroup, FoodFreshness foodFreshness, int quantity,
			Date expiryDate) {
		Item item = Item.getInstance(name, quantity, expiryDate);
	...
	/**
	 * Overloaded factory method to create an Item instance with food groups and
	 * freshness defined by enums and a string expiry date.
	 *
	 * @param name          The item's name.
	 * @param foodGroup     A set of FoodGroup enums.
	 * @param foodFreshness The FoodFreshness enum value.
	 * @param quantity      The quantity of the item.
	 * @param expiryDate    A string representing the item's expiry date in
	 *                      "dd-MMMM-yyyy" format.
	 * @return A new Item instance.
	 */
	public static Item getInstance(String name, FoodGroup foodGroup, FoodFreshness foodFreshness, int quantity,
	...
	/**
	 * Factory method to create an Item instance with a name, quantity, and expiry
	 * date.
	 *
	 * @param name       The name of the item.
	 * @param quantity   The quantity of the item.
	...
	public static Item getInstance(Item item) {
		return Item.getInstance(item.getName(), item.getFoodGroupTag(), item.getFoodFreshnessTag(), item.getQuantity(),
				item.getExpiryDate());
	}
	...
	public GenericTag<FoodFreshness> getFoodFreshnessTag() {
		// Directly return null if foodFreshnessTag is null, avoiding
	...
	/**
	 * Sets the item's quantity. Throws IllegalArgumentException if the new quantity
	 ...
	 * based on the item's name and expiry date.
	 *
	...
	/**
	 * Returns a string representation of the item. The string representation
	 * includes the item's name, food group tags, food freshness tag, quantity, and
	 * expiry date.
	 *
	...
	public String toString() {
		return "Item{" + "name='" + name + '\'' + ", foodGroupTags=" + foodGroupTag + ", foodFreshnessTag="
				+ foodFreshnessTag + ", quantity=" + quantity + ", expiryDate=" + expiryDate + '}';
	}

	/**
	 * Compares this item with the specified item for order. Ordering is first done
	 * by the expiry date and then by the item's name.
	 *
